...
int mem(unsigned long * args)
{
  ...
  if(shm)
  {
    ...
    {
      // eapp_print("i is %d, num is %d\n", i, shm[i]);
      sum += shm[i];
    }
    eapp_print("read shm: 0x%lx\n", sum);
  }
  eapp_print("end test shm\n");

  eapp_print("begin test mm_arg\n");
  int* mm_arg = (int*)args[13];
  unsigned long mm_arg_size = args[14];
  if(mm_arg)
  {
    unsigned long sum = 0;
    for(int i = 0; i < mm_arg_size/sizeof(int); ++i)
    {
      sum += mm_arg[i];
    }
    eapp_print("read mm_arg: 0x%lx\n", sum);
  }
  eapp_print("end test mm_arg\n");
