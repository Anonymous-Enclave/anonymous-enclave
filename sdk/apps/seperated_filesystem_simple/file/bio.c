...
static uint64 __handle = -1UL;
static void* __premapped_buf = 0;
uint64 get_handle(){
#ifdef RAMDISK
  if(__handle == -1UL){
    __handle = acquire_enclave("ramdisk");
    if(__handle == -1UL){
...
#else
  if(__handle == -1UL){
    __handle = acquire_enclave("persistency");
    if(__handle == -1UL){
      eapp_print("acquire persistency handle failed\n");
    }
  }
#endif
  return __handle;
...
/*
 * Same as bread, but we will always update the data in buf to zero
 * */
struct buf* bread_zero(uint dev, uint blockno)
{
  // eapp_print("bread block_no: %d\n",blockno);
  struct buf *b;
  b = bget(dev, blockno);
