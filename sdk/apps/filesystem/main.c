...
int EAPP_ENTRY main(){
  ...
  EAPP_RESERVE_REG;
  eapp_print("filesystem server begin to run\n");
  if(!inited){
    init();
    inited = 1;
  }
  ...
  case FSOPEN:
  {
    ...
    ret_arg.resp_val = fd;
    // eapp_print("server: FSOPEN, path: %s, fd: %d\n",arg->path,fd);
  ...
  case FSWRITE:
  {
    ...
    // eapp_print("server: write size: %d\n",write_size);
  ...
  default:
    eapp_print("error call_arg_type\n");
    ret_arg.req_vaddr = args[11];
    ret_arg.req_size = args[12];
    ret_arg.resp_val = -1;
    SERVER_RETURN(&ret_arg);
