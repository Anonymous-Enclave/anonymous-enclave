...
{
  //initialize pt_pages
  unsigned long pages = (totalram_pages % PTRS_PER_PTE) ? (totalram_pages/PTRS_PER_PTE + 1) : (totalram_pages/PTRS_PER_PTE);
  ...
  //pages: total pages to store the page list
...
char* alloc_pt_pgd_page()
{
  ...
  {
    SBI_CALL_4(SBI_SM_SET_PTE, SBI_PTE_MEMSET, __pa(free_page), 0, PAGE_SIZE);
  }
  else
  {
